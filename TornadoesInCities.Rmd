...
Chance that a tornado creates EF2 or worse damage given that it creates at least EF1 damage. The assumption is that, all else being equal, that chance will be higher with more targets in an area. Given the model of the expanding bull's eyes we would expect the ratio of EF2+/EF1+ tornadoes to be higher now then in the past. To the extent that that ratio is higher now for a given CBSA we label it has having experienced an expanding bull's eye.

Expanding bull's eye is a simple conceptual model to illustrate how spatiotemporal development changes found
in metropolitan regions have led to and will continue to place ever increasing number of built ‘‘targets’’ in harm's way.
...
ggplot() +
  ...
  labs(title = paste0("Approximate tracks of significant tornadoes\n", NameC),
       subtitle = paste0("Totals: 1951-1985 (purple): ", nT[1], 
                         paste0(", 1986-2020 (green): ", nT[2])))
```

Time series of ratio EF2+/EF1+ in 10-year moving averages.
```{r}
AreaTors.sf |>
  st_drop_geometry() |>
  group_by(yr) |>
...
( US_map <- ggplot(data = us_states) +
  ...
  labs(title = "Expanding bull's eyes",
       caption = "Gray regions are outside of core-based statistical areas or have had fewer than 15 tornadoes.") +
  theme_void() +
  theme(legend.title=element_blank()) )
```

Areas in blue have had a larger percentage of strong tornadoes relative to all tornadoes since 2014.
