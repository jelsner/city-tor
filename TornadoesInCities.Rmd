...
KS_Urban2020.sf <- urban_areas(year = 2020) |>
  st_crop(KS.sf)
...
tm_shape(KS.sf) +
  tm_borders() +
  tm_shape(KS_Urban2020.sf) +
  tm_borders() +
  tm_shape(KS_Urban2012.sf) +
  tm_borders(col = "red")
```

Get the tornado tracks. Tracks that recorded at least EF2 damage somewhere.
```{r}
KS_Torn.sf <- st_read(dsn = "1950-2019-torn-aspath", 
                      layer = "1950-2019-torn-aspath") |>
  st_transform(crs = st_crs(KS.sf)) |>
  filter(mag >= 0 & yr >= 2000 & st %in% c("OK", "KS", "CO", "MO", "NE"))
...
Mark all tornadoes that intersect urban areas of Kansas.
```{r}
Intersects <- KS_Urban2012.sf |>
  st_intersects(KS_Torn.sf) |>
  unlist()

KS_Torn.sf$row <- 1:nrow(KS_Torn.sf)

KS_Torn.sf <- KS_Torn.sf |>
  mutate(Urban = row %in% Intersects)

tm_shape(KS_Urban2012.sf) +
  tm_borders() +
  tm_shape(KS_Torn.sf[KS_Torn.sf$Urban, ]) +
  tm_lines(col = "red")
```
