...
Examining the expanding bull's eye model for tornado activity using changes in housing units

An empirical study of the expanding bull's eye model. https://chubasco.niu.edu/ebe.htm

Chance that a tornado creates at least EF2 level damage. The assumption is, that all else being equal, the chance will be higher with more targets to damage. The expanding bull's eye model predicts this chance to be higher now then in the past. 
...
Get API key from U.S. Census. Then
```{r}
...
Get the CBSA polygons and population for year 2020. Housing units (`B25001_001`) better than population (`B01003_001`). 
```{r}
...
CBSA2020.sf <- tidycensus::get_acs(geography = "cbsa",
                                   variables = "B25001_001",
                                   year = 2020,
                                   geometry = TRUE) |>
  sf::st_transform(crs = 32615) |>
  dplyr::mutate(LSAD = stringr::str_sub(NAME, start = -10),
                NAME = stringr::str_sub(NAME, end = -12),
                STUSPS = stringr::str_sub(NAME, start = -2, end = -1),
                Area2020 = sf::st_area(geometry),
                HU2020 = estimate,
                HUD2020 = HU2020 / Area2020)
                

CBSA2011.sf <- tidycensus::get_acs(geography = "cbsa",
                                   variables = "B25001_001",
                                   year = 2011,
                                   geometry = TRUE) |>
  sf::st_transform(crs = 32615) |>
  dplyr::mutate(LSAD = stringr::str_sub(NAME, start = -10),
                NAME = stringr::str_sub(NAME, end = -12),
                STUSPS = stringr::str_sub(NAME, start = -2, end = -1),
                Area2011 = sf::st_area(geometry),
                HU2011 = estimate,
                HUD2011 = HU2011 / Area2011)

df <- CBSA2011.sf |>
  sf::st_drop_geometry() |>
  dplyr::select(GEOID, Area2011, HU2011, HUD2011)

CBSA.sf <- CBSA2020.sf |>
  dplyr::inner_join(df, by = "GEOID")

CBSA.sf <- CBSA.sf |>
  dplyr::mutate(perChangeHU = (HU2020-HU2011)/HU2011 * 100)
```
...
CBSA.sf |> 
  st_drop_geometry() |>
  group_by(LSAD) |> 
  summarize(TH2011 = sum(HU2011), 
            TH2020 = sum(HU2020),
            perChange = (TH2020 - TH2011) / TH2011 * 100)
CBSA.sf |> 
  st_drop_geometry() |>
  filter(LSAD == "Micro Area") |>
  group_by(NAME) |> 
  summarize(TH2011 = sum(HU2011), 
            TH2020 = sum(HU2020),
            perChange = (TH2020 - TH2011) / TH2011 * 100) |>
  arrange(desc(perChange))

```

The number of housing units in 2020 are up about 4% for the micro areas and about 8% for the metro areas since 2011.

In the micro areas with changes in HU greater than 20% do we collectively see increases in the ratio of strong to weak tornadoes?
```{r}
CBSAx.sf <- CBSA.sf |>
#  dplyr::filter(LSAD == "Micro Area") |>
  dplyr::filter(perChangeHU > 0)
```
...
Torn.sf <- st_read(dsn = here::here("data", "1950-2020-torn-aspath"), 
  ...
  st_transform(crs = st_crs(CBSA.sf)) |>
  filter(yr >= 1951) |>
#  filter(yr >= 1995) |>
#  filter(yr >= 2007) |>
  mutate(Late = yr >= 1986) |>
...
```{r}
#NameC <- "St. Louis, MO-IL"
#NameC <- "Fort Payne, AL"
#NameC <- "Tupelo, MS"
#NameC <- "Dodge City, KS"
#NameC <- "Batesville, AR"
#NameC <- "Fergus Falls, MN"
...
See {stars.Rmd} for creating a space-time object.
```{r}
Intersects <- CBSA.sf |>
...
Intersects.m <- CBSA.sf |>
  sf::st_intersects(Torn.sf, sparse = FALSE)

CBSA.sf$nT = rowSums(Intersects.m)

Torn2.sf <- Torn.sf |>
  dplyr::filter(mag >= 2)
Intersects <- CBSA.sf |>
  sf::st_intersects(Torn2.sf) |>
  unlist()
AreaTors.sf <- Torn2.sf[Intersects, ]

Intersects.m <- CBSA.sf |>
  sf::st_intersects(Torn2.sf, sparse = FALSE)

CBSA.sf <- CBSA.sf |>
  mutate(nT2 = rowSums(Intersects.m),
         ratio = nT2/nT,
         HUChange = HU2020 - HU2011,
         HUChangeP = HUChange/HU2011,
         HUDChange = HUD2020 - HUD2011,
         HUDChangeP = HUDChange/HUD2011)
```

Graph the relationship between HU change and ratio
```{r}
CBSA2.sf <- CBSA.sf |>
  dplyr::filter(nT >= 20) |>
  dplyr::filter(HUChange > 0)

ggplot(data = CBSA2.sf,
       mapping = aes(x = HU2020, y = ratio)) +
  geom_point() +
  geom_text(mapping = aes(label = NAME)) +
  scale_x_log10() +
  geom_smooth(method = lm) +
  facet_wrap(~ LSAD, scales = "free_x") 
```

Repeat for early and late.
```{r}
EF <- 2
...
CBSA.sf$nTearly <- rowSums(Intersects.m)
...
CBSA.sf$nTlate <- rowSums(Intersects.m)
...
CBSA.sf$nT2early <- rowSums(Intersects.m)
...
CBSA.sf$nT2late <- rowSums(Intersects.m)
```
...
table(CBSA2.sf$LSAD, CBSA2.sf$EBEtrue)
```
...
Combine the two data sets and compute the frequency spectra
...
FFT on annual tornado counts using the historical data
```{r}
Old.df$residuals <- residuals(loess(nT ~ Year, data = Old.df))

fft <- torch::torch_fft_fft(as.numeric(scale(Old.df$residuals)))

num_samples <- nrow(Old.df)
nyquist_cutoff <- ceiling(num_samples / 2) # highest discernible frequency
bins_below_nyquist <- 0:nyquist_cutoff

sampling_rate <- 1 # per year
frequencies_per_bin <- sampling_rate / num_samples
frequencies <- frequencies_per_bin * bins_below_nyquist

data.frame(f = frequencies, y = as.numeric(fft[1:(nyquist_cutoff + 1)]$abs())) |>
ggplot(mapping = aes(f, y)) +
  geom_line() +
  xlab("frequency (per year)") + ylab("magnitude") +
  ggtitle("Spectrum of annual tornado counts")

( strongest <- torch::torch_topk(fft[1:(nyquist_cutoff/2)]$abs(), 6) )
( important_freqs <- frequencies[as.numeric(strongest[[2]])] )
( num_observations_in_season <- 1/important_freqs  )
```

FFT on annual tornado counts using the modern data
```{r}
...
Torn.df$residuals <- residuals(loess(nT ~ Year, data = Torn.df))

fft <- torch::torch_fft_fft(as.numeric(scale(Torn.df$residuals)))

num_samples <- nrow(Torn.df)
nyquist_cutoff <- ceiling(num_samples / 2) # highest discernible frequency
bins_below_nyquist <- 0:nyquist_cutoff
...
ggplot(mapping = aes(f, y)) +
  geom_line() +
  xlab("frequency (per year)") + ylab("magnitude") +
  ggtitle("Spectrum of modern tornado data")
...
FFT on the combined data
```{r}
AllTorn.df <- Old.df[Old.df$Year < 1950, ] |>
  dplyr::bind_rows(Torn.df)

ggplot(data = AllTorn.df,
       mapping = aes(x = Year, y = nT)) +
  geom_line() +
  geom_smooth()

AllTorn.df$residuals <- residuals(loess(nT ~ Year, data = AllTorn.df))

ggplot(data = AllTorn.df,
       mapping = aes(x = Year, y = residuals)) +
  geom_line()

fft <- torch::torch_fft_fft(as.numeric(scale(AllTorn.df$nT)))

num_samples <- nrow(AllTorn.df)
nyquist_cutoff <- ceiling(num_samples / 2) # highest discernible frequency
bins_below_nyquist <- 0:nyquist_cutoff
...
ggplot(mapping = aes(f, y)) +
  geom_line() +
  xlab("frequency (per year)") + ylab("magnitude") +
  ggtitle("Spectrum of all tornado data")

( strongest <- torch::torch_topk(fft[1:(nyquist_cutoff/2)]$abs(), 4) )
( important_freqs <- frequencies[as.numeric(strongest[[2]])] )
( num_observations_in_season <- 1/important_freqs  )
```


We now pass these to `feasts::STL()`, to obtain a decomposition into trend, components, and remainder
```{r}
library(feasts)

AllTorn.df |>
  model(STL(nT ~ season(period = 142) + season(period = 47) +
                 season(period = 9))) |>
  components() |>
  autoplot()
```
...
x <- x + .2*rnorm(1000) # add some noise

df <- data.frame(x = x)
w <- analyze.wavelet(df, 
                      "x", 
                       loess.span = 0,
                       dt = 1, 
                       dj = 1/250,
...
No consistent oscillation across the period of record
...
Example of using time series data frame (`tsibble`). Get ENSO data
https://blogs.rstudio.com/ai/posts/2023-01-19-torchwavelets/
```{r}
download.file("https://bmcnoldy.rsmas.miami.edu/tropics/oni/ONI_NINO34_1854-2022.txt",
              destfile = here::here("data", "ONI_NINO34_1854-2022.txt"))

enso <- readr::read_table(here::here("data", "ONI_NINO34_1854-2022.txt"), 
                                     skip = 9) |> # skip first 9 rows
  dplyr::mutate(t = tsibble::yearmonth(as.Date(paste0(YEAR, "-", `MON/MMM`, "-01")))) |>
  dplyr::select(t, enso = NINO34_MEAN) |>
  dplyr::filter(t >= tsibble::yearmonth("1950-01"), t <= tsibble::yearmonth("2022-09")) |>
  tsibble::as_tsibble(index = t)

enso

fft <- torch::torch_fft_fft(as.numeric(scale(enso$enso)))
...
Repeat using the historical significant tornado data by month
...
Old.df <- OldTorn.sf |>
  ...
  dplyr::summarize(nT = dplyr::n()) |>
  dplyr::mutate(t = tsibble::make_yearmonth(year = Year, month = Month)) |>
  tsibble::as_tsibble(index = t)

fft <- torch::torch_fft_fft(as.numeric(scale(Old.df$nT)))
...
ggplot(mapping = aes(f, y)) +
  geom_line() +
  xlab("frequency (per year)") + ylab("magnitude") +
  ggtitle("Spectrum of historical tornado data")
```

Strongest components
```{r}
( strongest <- torch::torch_topk(fft[1:(nyquist_cutoff/2)]$abs(), 3) )
```
...
Expressed the periodicity in months
```{r}
