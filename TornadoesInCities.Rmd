...
All core-based statistical areas. See {stars.Rmd} for creating a space-time object.
```{r}
Intersects <- CBSA.sf |>
  st_intersects(Torn.sf) |>
  unlist()
...
EF ratings rural versus urban impacts. Whether a tornado hits an urban area or not determines to some extent the amount damage that occurs. Here we attempt to quantify the magnitude of this effect. Specifically we ask how much less likely (on average) is EF3 (or worse) damage in rural areas compared with urban areas?
...
```{r}
library(MASS)
m1 <- polr(maxEFf ~ Urban, data = df)
brant::brant(m1)
m2 <- polr(maxEFf ~ Urban + PathAreaS, data = df)
brant::brant(m2)
```

Testing proportional odds assumption: https://medium.com/evangelinelee/brant-test-for-proportional-odds-in-r-b0b373a93aa2

Failed the proportional odds assumption. Must use a multinomial logistic regression. https://stats.idre.ucla.edu/r/dae/multinomial-logistic-regression/
...
Interpreting the coefficients https://stats.idre.ucla.edu/other/mult-pkg/faq/ologit/
