...
Get the modern tornado data since 2007 (start of EF rating).
```{r}
Torn.sf <- st_read(dsn = "1950-2019-torn-aspath", 
                   layer = "1950-2019-torn-aspath") |>
  filter(yr >= 2007)
...
df <- Torn.sf |>
  st_drop_geometry() |>
  group_by(yr) |>
  summarize(nF2 = sum(mag == 2),
            nF3p = sum(mag == 3 | mag == 4 | mag == 5),
            ratio = nF3p / nF2) 
  ggplot(data = df, 
         mapping = aes(x = yr, y = ratio)) +
  geom_line()
```
